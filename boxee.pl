...
$boxee = new xPL_Media("c99org-boxee.sigbox"); # noloop
...
sub boxee_change_hook {
	...
	if(state_changed $boxee) {
		...
		if(($boxee->state eq 'stop' or $boxee->state eq 'pause') and $livingroom_light->state eq 'off') {
			set $livingroom_light 'on';
		}
	} elsif($boxee->state eq 'play' and defined $boxee->title() and defined $boxee->artist() and $boxee->kind() eq "audio" and length($boxee->artist()) > 0) {
		speak("app=boxee " . $boxee->title() . " by " . $boxee->artist());
	}

	if($boxee->state eq 'play' and $boxee->kind() eq 'video' and $boxee->duration() > 3600 and $livingroom_light->state eq 'on') {
